...
class SparkPositionDeltaWrite implements DeltaWrite, RequiresDistributionAndOrdering {
  ...
  SparkPositionDeltaWrite(SparkSession spark, Table table, Command command, SparkMergeOnReadScan scan,
  ...
  private static class PositionDeltaWriteFactory implements DeltaWriterFactory {
    ...
    public DeltaWriter<InternalRow> createWriter(int partitionId, long taskId) {
      ...
      } else if (dataSpec.isUnpartitioned()) {
        return new UnpartitionedDeltaWriter(
            writerFactory, dataFileFactory, deleteFileFactory, io, ctx,
            dataSpec, specs, partitionRowWrapper, partitionProjections);
  ...
  private static class PartitionedDeltaWriter implements DeltaWriter<InternalRow> {
    ...
    PartitionedDeltaWriter(SparkFileWriterFactory writerFactory, OutputFileFactory dataFileFactory,
                           OutputFileFactory deleteFileFactory, FileIO io, Context ctx,
                           PartitionSpec dataSpec, Map<Integer, PartitionSpec> specs,
                           InternalRowWrapper deletePartitionRowWrapper,
                           Map<Integer, StructProjection> deletePartitionProjections) {

      PartitioningWriter<InternalRow, DataWriteResult> insertWriter;
      if (ctx.fanoutWriterEnabled()) {
        insertWriter = new FanoutDataWriter<>(
            writerFactory, dataFileFactory, io,
            ctx.dataFileFormat(), ctx.targetDataFileSize());
      } else {
      ...
      this.dataPartitionKey = new PartitionKey(dataSpec, ctx.dataSchema());
      this.internalRowDataWrapper = new InternalRowWrapper(ctx.dataSparkType());
      this.deletePartitionRowWrapper = deletePartitionRowWrapper;
      this.deletePartitionProjections = deletePartitionProjections;
      this.specIdOrdinal = ctx.metadataSparkType().fieldIndex(MetadataColumns.SPEC_ID.name());
      this.partitionOrdinal = ctx.metadataSparkType().fieldIndex(MetadataColumns.PARTITION_COLUMN_NAME);
      this.fileOrdinal = ctx.deleteSparkType().fieldIndex(MetadataColumns.FILE_PATH.name());
      this.positionOrdinal = ctx.deleteSparkType().fieldIndex(MetadataColumns.ROW_POSITION.name());
  ...
  private static class UnpartitionedDeltaWriter implements DeltaWriter<InternalRow> {
    private final PositionDeltaWriter<InternalRow> delegate;
    private final FileIO io;
    private final PartitionSpec dataSpec;
    private final Map<Integer, PartitionSpec> specs;
    ...
    UnpartitionedDeltaWriter(SparkFileWriterFactory writerFactory, OutputFileFactory dataFileFactory,
                             OutputFileFactory deleteFileFactory, FileIO io, Context ctx,
                             PartitionSpec dataSpec, Map<Integer, PartitionSpec> specs,
                             InternalRowWrapper deletePartitionRowWrapper,
                             Map<Integer, StructProjection> deletePartitionProjections) {
